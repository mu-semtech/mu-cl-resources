...
(labels ((delete-query (resource-uri link)
           (let ((pattern (if (inverse-p link)
                              (list (s-var "s")
                                    (ld-link link)
                                    resource-uri)
                              (list resource-uri
                                    (ld-link link)
                                    (s-var "s")))))
             (sparql:update (format nil "DELETE { ~{~A~,^ ~} } WHERE { ~{~A~,^ ~}. VALUES ?type { ~{~A ~}}. ?s a ?type. }"
                                    pattern
                                    pattern
                                    (ld-subclasses (referred-resource link))))))
                     ...
                     do
                        (loop for target-super in target-resource-super-tree
                              when (find inverse-link (direct-has-one-links target-super))
                                do
                                   (delete-query (s-url (node-url new-item-spec))
                                                 inverse-link))))))
      ...
      (let ((linked-resource (referred-resource link))
        ...
        (if (and resource-specification
            ...
            (let* ((new-linked-uuid (jsown:val resource-specification "id"))
              ...
              (sparql:with-update-group
                (delete-query (s-url resource-uri) link)
...
(defparameter *delete-call-relations-to-delete-cache* (lhash:make-castable :test 'eq)
  "Cache containing the relations to delete for a specific resource.")
                ...
                (sparql:delete
                    (apply #'concatenate 'string
                           (loop for triple-clause
                                 ...
                                 `(;; uuid
                                   ...
                                   ;; unhandled inverse properties
                                   ,@(loop for (property value) in additional-incoming-relations
                                           collect `(,(s-var "s") ,property ,value)))
                    ...
                    (concatenate 'string
                     ...
                     (format nil "~{~&~4tOPTIONAL {~{~A ~A ~A~}.}~%~}"
                             (loop for (property value) in additional-incoming-relations
                                   if (s-inv-p property)
                                     collect `(,value ,(s-inv property) ,(s-var "s"))
                                   else
                                     collect `(,(s-var "s") ,property ,value)))))))))
        (respond-no-content)))))
