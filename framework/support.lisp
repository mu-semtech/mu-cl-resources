...
(defun resource-subtypes-of-supertype (resource &key test)
  "Uses TEST on each supertype of RESOURCE.  Returns the highest superclass
which matches test and all of its subtypes."
  (let ((super-resource (find-if test (reverse (flattened-class-tree resource)))))
    (subclass-resources super-resource)))

(defun cache-clear-relation (resource relation)
  ;; find which (super-)type defines the relationship, then clear any
  ;; subtype of that (parent) type.
  (flet ((direct-json-relation-p (super-resource)
...
RELATION."
           (find (json-property-name relation)
                 ...
                 :test #'string=)))
    (dolist (resource-to-clear (resource-subtypes-of-supertype
                                resource
                                :test #'direct-json-relation-p))
    ...
    (dolist (inverse-relation-spec (inverse-links relation))
      (let ((inverse-resource (getf inverse-relation-spec :resource))
        ...
        (dolist (resource-to-clear (subclass-resources inverse-resource))
          (add-clear-key :ld-resource (expanded-ld-class resource-to-clear)
